...
(define (layer-walk get-layer start next test action limit ignore-first)
  (let loop ((i start)
             (left limit))
    ...
    (lambda (target)
      (let* ((bindings (get-bindings (cadr opts)))
             (apply-effects (apply-effects-simple img (list-ref opts 3)
                                                  bindings #f)))
        (if (car opts) (apply-effects source target))
        (let ((new (car (gimp-layer-copy source FALSE))))
          ...
          (if (not (car opts)) (apply-effects new target)))))))
...
(define (animstack-common delta pos actionfn)
  (lambda (img layer opts . params)
    (let* ((limit (if (null? params) #f (car params)))
      ...
      (layer-walk getter start next is-untagged? action limit #t))
  ...
  (let* ((roll-list (if (>= roll-mode 0) 
    ...
    (lambda (target)
      (let* ((pick (- roll-length 1 (modulo (animstack-get-count) roll-length)))
        ...
        (if (car opts) (apply-effects temp-layer target))
...
(define (animstack-sampler img layer opts . params)
  (with-params
   ...
   (let* ((start (get-start-position img layer opts))
     ...
     (if (or (not count) (= count 0)) (set! count (max (sampler-count-frames-above img layer) 1)))
...
(define (animstack-process-tag img layer tag generator-alist before-effects during-effects extra-opts)
  (let* ((parse-tagname (lambda (str)
                          (if (and (> (string-length str) 0)
                                   (char=? (string-ref str 0) #\.))
                              (cons (substring str 1 (string-length str)) #f)
                              (cons str #t)))) 
         (tag-assocs `(("bg" ,animstack-bg) 
                       ("fg" ,animstack-fg)
                       ("copy" ,animstack-copy)
                       ("roll" ,animstack-roll)
      ...
      (let* ((curtag (car rest))
        ...
        (if (not (and (= (length namesplit) 2) (symbol? var)))
            ...
            (let* ((defnlist (parse-generator-defn (cadr namesplit)))
                   (gen-assoc (assoc (caddr defnlist) gen-assocs)))
              (and gen-assoc
...
(define (process-effect-tag tag normal)
  (let* ((tag-defn (car tag))
         ...
         (tag-range #f)
         (tag-name #f)
         (effect-assocs `(("scatter" ,animstack-scatter)
                          ("move" ,animstack-move)
    ...
    (let ((tag-assoc (assoc tag-name effect-assocs)))
      (and tag-assoc
